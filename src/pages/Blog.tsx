...
const Blog = () => {
  ...
  const filteredPosts = blogPosts.filter(post => {
    ...
    return matchesCategory && matchesSearch && !post.is_featured;
  });
      ...
      {featuredPost && (
        <section className="py-16 bg-muted/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Card 
              className="overflow-hidden shadow-elegant hover:shadow-2xl transition-all duration-500 group cursor-pointer"
              onClick={() => navigate(`/blog/${featuredPost.slug}`)}
            >
              <div className="grid grid-cols-1 lg:grid-cols-2">
                {/* Image */}
                <div 
                  className="relative h-64 lg:h-auto bg-gradient-primary bg-cover bg-center"
                  style={featuredPost.featured_image_url ? { backgroundImage: `url(${featuredPost.featured_image_url})` } : {}}
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-bold text-foreground font-playfair">
              {loading ? 'Loading...' : `Latest Articles (${filteredPosts.length})`}
            </h2>
          </div>
                ...
                return (
                  <div key={post.id} onClick={() => navigate(`/blog/${post.slug}`)}>
                    ...
                    <div className="space-y-4 h-full flex flex-col">
                      {/* Image */}
                      <div 
                        className="relative h-48 bg-gradient-primary rounded-lg overflow-hidden bg-cover bg-center"
                      ...
                      <div className="space-y-4 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm font-medium">Grupo Sineva</span>
                          </div>
